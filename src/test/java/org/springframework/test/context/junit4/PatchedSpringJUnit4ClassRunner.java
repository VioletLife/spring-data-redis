...
/**
 * Temporarily imported and adjusted from (spring-test-4.0.6) to enable more dynamic checks for test enablement.
 * @see #isTestClassIgnored(Class)
 * @see #getDescription()
 * @see #run(RunNotifier)
 * 
 * <p>
...
public class PatchedSpringJUnit4ClassRunner extends BlockJUnit4ClassRunner {

	private static final Log logger = LogFactory.getLog(PatchedSpringJUnit4ClassRunner.class);
	 ...
	 */
	public PatchedSpringJUnit4ClassRunner(Class<?> clazz) throws InitializationError {
		super(clazz);
	...
	/**
	 * Checks whether tests of the given {@link Class} are ignored.
	 * Subclasses can override this method in order to implement custom checks.
	 * 
	 * @param testClass
	 * @return
	 */
	protected boolean isTestClassIgnored(Class<?> testClass) {
		return !ProfileValueUtils.isTestEnabledInThisEnvironment(testClass);
	}
	...
	public Description getDescription() {
		...
		//patched for SD Redis Tests
		if (isTestClassIgnored(getTestClass().getJavaClass())) {
